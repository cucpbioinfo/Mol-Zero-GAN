...
class LatentGANTrainer(MosesTrainer):
    ...
    def get_collate_fn(self, model):
        ...
        def collate(data):
            tensors = torch.tensor(
                [t for t in data], dtype=torch.float64, device=device
            )
            return tensors
    ...
    def fit(self, model, train_data, val_data=None):
        ...
        if self.config.heteroencoder_version == "new":
            ...
            path = "{}/moses/latentgan/heteroencoder_models/new_model".format(
                currentDirectory
            )
            encoder_checkpoint_path = (
                "{}/moses/latentgan/heteroencoder_models/checkpoints/".format(
                    currentDirectory
                )
            )
            ...
            heteroencoder_mols = [
                Chem.rdchem.Mol.ToBinary(Chem.MolFromSmiles(smiles))
        ...
        print("Training GANxxxx.")  ######

        mols_in = []
        print("len train_data", len(train_data))
        for smiles in train_data:

            mol_in.append(Chem.rdchem.Mol.ToBinary(Chem.MolFromSmiles(smiles)))
        ...
        train_loader = self.get_dataloader(
            model, LatentMolsDataset(latent_train), shuffle=True
        )
        val_loader = (
            None
            if val_data is None
            else self.get_dataloader(
